...
static GLPlayer *_shareInstance;

+ (instancetype)shareInstance {
    if (!_shareInstance) {
        _shareInstance = [[GLPlayer alloc] init];
    }
...
- (void)playWithURL: (NSURL *)url{
    ...
    if ([self.url isEqual:url]) {
        ...
        if (self.state == GLPlayerStatePlaying) {
            return;
        }
        if (self.state == GLPlayerStatePause) {
            [self resume];
            return;
        }
        if (self.state == GLPlayerStateLoading) {
    ...
    CMTime interval = CMTimeGetSeconds(item.asset.duration) > 60 ? CMTimeMake(1, 1) : CMTimeMake(1, 30);

    [self.player addPeriodicTimeObserverForInterval:interval queue:nil usingBlock:^(CMTime time) {
        __strong typeof(self) strongSelf = weakSelf;
        if (strongSelf.state == GLPlayerStateStopped) {
            return;
        }
        [strongSelf timeObserver:time];
    }];

}


- (void)timeObserver:(CMTime)time{
    CGFloat playProgress = CMTimeGetSeconds(time) / CMTimeGetSeconds(self.player.currentItem.asset.duration);
    
    if (self.progressBlock){
        self.progressBlock(playProgress,CMTimeGetSeconds(time),CMTimeGetSeconds(self.player.currentItem.asset.duration));
    }
}
...
- (void)resume{
    ...
    if (self.player && self.player.currentItem.playbackLikelyToKeepUp) {
        _isUserPause = NO;
        self.state = GLPlayerStatePlaying;
    }
...
- (void)stop{ 
    ...
    [self clearObserver:self.player.currentItem];
    self.player = nil;
    self.state = GLPlayerStateStopped;
}
...
- (double)duration {
    double time = CMTimeGetSeconds(self.player.currentItem.asset.duration);
    if (isnan(time)) {
        return 0;
    }
    return time;
...
- (void)setState:(GLPlayerState)state {
    _state = state;
    if (self.stateChange) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        switch (status) {
            ...
            {
                NSLog(@"数据准备失败, 无法播放");
                self.state = GLPlayerStateFailed;
                break;
            }
            ...
            {
                NSLog(@"未知");
                self.state = GLPlayerStateUnknown;
                break;
            }
        }
    ...
    if ([keyPath isEqualToString:@"playbackLikelyToKeepUp"]) {
        ...
        if (playbackLikelyToKeepUp) {
            ...
            NSLog(@"数据不够播放");
            self.state = GLPlayerStateLoading;
        }
    }
